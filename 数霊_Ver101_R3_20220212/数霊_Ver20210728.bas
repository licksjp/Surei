...
290 Version$ = "1.0.1.2022.02.12(R3)"
...
390 '変数領域
400 dim bufline$(181 * 2+3),bufcount$(20):count = 0:dim bufcount1$(20):dim bufcount2$(20):mode = 0:dim bufline2$(181+3,2):dim bufline3$(181*2+3):dim bufKaztama$(50)
...
3530 if No=4 then color rgb(255,255,255):play Voice_File_TopScreen$ + "Voice_Finish_Message.mp3":pause 2.052*1000:ui_msg"プログラムを終了します":cls 3:end
3540 if No > 4 or No=0 then goto Setting:
3550 '文字検索項目  ここから
3560 Moji_Search:
...
5410 if No=4 then color rgb(255,255,255):play Voice_File_TopScreen$+"Voice_Finish_Message.mp3":pause 2.052*1000:ui_msg "プログラムを終了します":cls 3:end
5420 '
...
5590 if No=3 then color rgb(255,255,255):play Voice_File_TopScreen$+"Voice_Finish_Message.mp3":pause 2.052*1000:ui_msg"プログラムを終了します":cls 3:end
...
5660 cls 3
5670 gload "data/Gazo/"+"KazutamaList1.png"
5680 print chr$(13)
5690 color rgb(255,0,0):print"登録ファイルはございません" + chr$(13)
5700 color rgb(0,0,0):print "スペースキーを押してください" + chr$(13)
5710 key$=input$(1)
5720 if key$ = " " then goto TopScreen:
5730 else
5740 'cls 3:print"登録ファイルがございません" + chr$(13)
5750 'else
5760 cls 3:buffer_list_count=Loadcount()
5770 talk"登録件数は"+str$(buffer_list_count)+"件です"
5780 gload Gazo$ + "KazutamaList1_Entry.png"
5790 color rgb(0,0,255)
5800 'print"数霊リスト  登録件数" + chr$(13)
5810 print chr$(13)
5820 color rgb(255,0,0)
5830 print "登録件数:";buffer_list_count;"件";chr$(13)
5840 color rgb(0,0,0)
5850 print "エンターキーを押してください" + chr$(13)
5860 key$ = input$(1)
5870 if key$ = chr$(13) then
5880 goto KotodamaList_page:
5890 'else
5900 'goto Surei_List1:
5910 endif
5920 'endif
5930 endif
5940 func Loadcount()
5950 n = 0
5960 open "data/Parsonal_list1.csv" for input as #1
5970 while eof(1) = 0
5980 line input #1,line$
5990 n = n + 1
6000 wend
6010 close #1
6020 count_line = n
6030 endfunc count_line
6040 func Kotodama$(buf$)
6050 b = len("数霊:")
6060 a$ = Mid$(buf$,b+1,len(buf$))
6070 for i=0 to 118
6080 if bufline3$(2 * i) ="数霊:" + a$  then
6090 moji_$ = bufline3$(2*i + 1)
6100 'moji_$ = bufline3$(21)
6110 'moji$ = a$
6120 endif
6130 next i
6140 'kotodama$ = a$
6150 kotodama$ = moji_$
6160 endfunc kotodama$
6170 KotodamaList_page:
6180 cls 3:font 48:m = 0
6190 gload Gazo$ + "Screen5_Surei_List.png"
6200 open "data/Parsonal_list1.csv" for input as #1
6210 while eof(1) = 0
6220 input #1,line$
6230 bufKaztama$(m) = line$
6240 'input #1,line$
6250 'bufKaztama$(m,2*m + 1) = line$
6260 m = m + 1
6270 wend
6280 close #1
6290 l = 0:
6300 while buffer_list_count > l
6310 cls
6320 color rgb(255,0,0)
6330 'print "数霊　リスト" + chr$(13)
6340 print chr$(13)
6350 color rgb(255,255,255)
6360 n3 = l:n3 = n3 + 1
6370 print "番号:";n3;chr$(13)
6380 print bufKaztama$(2*l) + chr$(13)
6390 print bufKaztama$(2*l + 1) + chr$(13)
6400 'Kmoji$ = Kotodama$(str$(36))
6410 Kmoji$=moji$
6420 print chr$(13) + chr$(13)
6430 color rgb(0,0,0)
6440 print "スペース:次へ行く" + chr$(13)
6450 print "エンターキー:トップ画面" + chr$(13)
6460 key$ = input$(1)
6470 if key$ = " " then
6480 l = ((l + 1) Mod buffer_list_count)
6490 else
6500 if key$ = chr$(13) then
6510 goto TopScreen:
6520 else
6530 goto KotodamaList_page:
6540 endif
6550 endif
6560 wend
6570 '
6580 '結果表示２ Menu1
6590 Surei_Result4:
6600 cls 3:font (48-16)
6610 'buffer2$="":buffer3$="":buffer4$="":buffer5$="":buffer$=""
6620 for i=0 to 181 + 3 - 1
6630 if i = count  then
6640 buffer$ = bufline2$(count + 4,0)
6650 endif
6660 next i
6670 if len(buffer$) > 23 and len(buffer$) <= 45 then
6680 buffer2$ = Mid$(buffer$,1,24)
6690 buffer3$ = Mid$(buffer$,25,(len(buffer$) - 25))
6700 buffer$ = buffer2$ + chr$(13) + buffer3$ + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13)
6710 else
6720 if len(buffer$) <= 23 then
6730 buffer2$ = Mid$(buffer$,1,22)
6740 buffer$ = buffer2$ + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13)+chr$(13)
6750 else
6760 if len(buffer$) > 45 and len(buffer$) <= 64 then
6770 buffer2$ = Mid$(buffer$,1,22)
6780 buffer3$ = Mid$(buffer$,23,20)
6790 buffer4$ = Mid$(buffer$,43,(len(buffer$)-43))
6800 buffer$ = buffer2$ + chr$(13) + buffer3$ + chr$(13) + buffer4$ + chr$(13) + chr$(13) + chr$(13) + chr$(13) + chr$(13)+chr$(13)+chr$(13)
6810 else
6820 if len(buffer$) > 64 and len(buffer$) <= 81 then
6830 buffer2$ = Mid$(buffer$,1,23)
6840 buffer3$ = Mid$(buffer$,24,20)
6850 buffer4$ = Mid$(buffer$,42,20)
6860 buffer5$ = Mid$(buffer$,63,(len(buffer$)-62))
6870 buffer$ = buffer2$ + chr$(13) + buffer3$  + chr$(13) + buffer4$  + chr$(13) + buffer5$ + chr$(13) + chr$(13) + chr$(13)
6880 if len(buffer$) > 81 and len(buffer$) <= 110 then
6890 buffer2$ = Mid$(buffer$,1,21)
6900 buffer3$ = Mid$(buffer$,22,20)
6910 buffer4$ = Mid$(buffer$,42,20)
6920 buffer5$ = Mid$(buffer$,62,20)
6930 buffer6$ = Mid$(buffer$,82,(len(buffer$) - 82))
6940 buffer$ = buffer2$ + chr$(13) + buffer3$ + chr$(13) + buffer4$ + chr$(13) + buffer5$ + chr$(13) + buffer6$ + chr$(13) + chr$(13) + chr$(13)
6950 endif
6960 endif
6970 endif
6980 endif
6990 endif
7000 'cls 3
7010 gload Gazo$ + "Screen1_Result2.png"
7020 color rgb(255,0,0)
7030 'print "診断結果 2" + chr$(13)
7040 print chr$(13)
7050 color rgb(127,255,212)
7060 print "入力文字:";buffer_moji$;chr$(13)
7070 print "数霊:";count;chr$(13)
7080 color rgb(255,255,255)
7090 print "数霊の説明:"+chr$(13)
7100 print buffer$
7110 color rgb(0,0,0)
7120 print "エンターキー:トップ,S:保存する" + chr$(13)
7130 key$=input$(1)
7140 if key$ = chr$(13) then
7150 buffer$="" : buffer_moji$="" : count=0:goto TopScreen:
7160 else
7170 if key$="s" or key$="S" then
7180 s=Save():ui_msg"保存しました":goto Surei_Result4:
7190 else
7200 talk"無効な文字です":goto Surei_Result4:
7210 endif
7220 endif
