...
'*************************************************
Hiragana = check_hiragana(buffer_moji$)
'1.入力した文字が全部カタカナの時
'結果の画面ラベル(Surei_Result)に飛ぶ
'moji_length = Len(buffer_moji$)
if (Hiragana = 0) then
'結果の画面にいく
'1.平仮名が交じっていなければ結果の画面に行く
goto Surei_Result:
else
'***********************************************************************
'2.無効な文字(カタカナAlphabet大文字 and 小文字 以外が交じってる時)    *
'エラー表示を出して、                                                  *
'***********************************************************************
pause 30
ui_msg "無効な文字が交じってるので数霊の有効な文字で入れ直してください!!"
'2.入力文字の入力に戻る
goto Surei_Input:
endif
'************************************************
...
'*********************************************************************************************************************
'case 2:
'No = 2:sp_on 5,0:sp_on 4,0:sp_on 6,0:pause 100:sp_on 4,0:sp_on 6,1:locate 0,5:print "                    ":locate 0,5:print"どうしますか？":locate 0,7:print "                            ":locate 0,7:print " :1.終了してトップに戻る":locate 0,9:print "                  ":locate 0,9:print " :2をやる":Button_count=1:goto Select_Joypad_Kazutama_Save:
'**********************************************************************************************************************
'2.選択肢 3を押したときの処理
'case 2:
'No=2
...
'*********************************************************************************************************************
'case 2:
'No = 2:sp_on 5,0:sp_on 4,0:sp_on 6,0:pause 100:sp_on 4,0:sp_on 6,1:locate 0,5:print "                    ":locate 0,5:print"どうしますか？":locate 0,7:print "                            ":locate 0,7:print " :1.終了してトップに戻る":locate 0,9:print "                  ":locate 0,9:print " :やる":Button_count=1:goto Select_Joypad_Kazutama_Save:
'**********************************************************************************************************************
'2.選択肢 3を押したときの処理
'case 2:
'No=2
...
'*****************************************************
Dim buf_Alphabet_small$(26):moji_count=0:Moji_length=0
'アルファベットのチャート
Dim buf_Alphabet_large$(26)
...
'◎.2.入力した文字が 2文字以上のとき
'その文字数分の文字数の配列を作る。
Dim Moji_count$(Moji_length)
'*************************************************************
'入力した文字が2文字以上のとき
'1文字ずつ、Moji_count()に一文字ずつ代入
'例 入力した文字がAppleのとき
'Moji_count$(0)='A'
'Moji_count$(1)='p'
'Moji_count$(2)='p'
...
'***********************************************************
if (Moji_length > 1) then
for i = 0 to Moji_length - 1
Moji_count$(i) = Mid$(buf$,1,i)
next i
endif
'入力した文字にアルファベットの小文字がまじってるか調べる。
for n = 0 to 25
for i = 0 to Moji_length - 1
'************************************************************
'入力した文字にアルファベットの小文字があるとき。
'1.アルファベットの小文字のチャートに入力した文字が
'一致したときの処理。
'************************************************************
if (buf_Alphabet_small$(n) = Moji_count$(i)) then
'2.Alphabet_moji_countを1つ増やす。
Alphabet_moji_count = Alphabet_moji_count + 1
endif
next i
next n
...
'******************************************************
'アルファベットの小文字があれば1を返す。
'アルファベットの小文字があれば0 を返す
'******************************************************
if (Alphabet_moji_count > 0) then
'******************************************************
'1.アルファベットの小文字がある時なので1を返す。      *
'******************************************************
Alphabet_count = 1
else
'*******************************************************
'2.アルファベットの小文字が無いときなので0を返す。     *
'*******************************************************
Alphabet_count = 0
endif
'******************************************************
'作った配列の削除処理                                 *
'作った配列(Moji_count$(Moji_length))を消しておく。   *
'******************************************************
'erase Moji_count$
...
'********************************************************
'1.変数定義
Dim Hiragana$(75):moji$ = "":Moji_count = 0:moji_flag = 0
'ひらがなのチャート
Hiragana$ = {"あ","い","う","え","お","か","き","く","け","こ","さ","し","す","せ","そ","た","ち","つ","て","と","な","に","ぬ","ね","の","は","ひ","ふ","へ","ほ","ま","み","む","め","も","や","ゆ","よ","ら","り","る","れ","ろ","わ","を","ん","が","ぎ","ぐ","げ","ご","ざ","じ","ず","ぜ","ぞ","だ","ぢ","づ","で","ど","ば","び","ぶ","べ","ぼ","ぱ","ぴ","ぷ","ぺ","ぽ","ゃ","ゅ","ょ","U"}
'2.その他　濁音など
'Total:46 + 25 + 3 = 74文字
'
'1.入力した文字数を調べる。
